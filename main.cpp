...
int main() {
    ...
    while (window.isOpen()) {
        ...
        projectiles.erase(std::remove_if(projectiles.begin(), projectiles.end(),
            [](Projectile* p) { if (!p->isActive()) { delete p; return true; } else return false; }), projectiles.end());

        enemyProjectiles.erase(std::remove_if(enemyProjectiles.begin(), enemyProjectiles.end(),
            [](Projectile* p) { if (!p->isActive()) { delete p; return true; } else return false; }), enemyProjectiles.end());
...
//g++ main.cpp Player.cpp Object.cpp Enemy.cpp EasyEnemy.cpp Projectile.cpp Upgrade.cpp SpeedUpgrade.cpp -lsfml-graphics -lsfml-window -lsfml-system -std=c++11