...
Projectile::Projectile(sf::Shape* projectileShape, sf::Vector2f originPosition,
...
sf::Shape* Projectile::cloneShape(const sf::Shape* shape) {
  if (const sf::CircleShape* circle =
          dynamic_cast<const sf::CircleShape*>(shape)) {
    return new sf::CircleShape(*circle);
  } else if (const sf::RectangleShape* rect =
                 dynamic_cast<const sf::RectangleShape*>(shape)) {
